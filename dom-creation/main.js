...
function renderPokemon(pokemon) {
  ...
  $hTwoName.textContent = pokemon.name;
  $divPokemonCard.appendChild($hTwoName);
  const $hThreeNumber = document.createElement('h3');
  $hThreeNumber.textContent = pokemon.number;
  $divPokemonCard.appendChild($hThreeNumber);
  const $pDescription = document.createElement('p');
  $pDescription.textContent = pokemon.description;
  $divPokemonCard.appendChild($pDescription);
...
for (let i = 0; i < pokedex.length; i++) {
  ...
  $divRow.appendChild(renderPokemon($pokemon));
}
